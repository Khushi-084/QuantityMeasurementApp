...
{
    /// <summary>
    /// UC15/UC16: Controller Layer — Menu Driven with User Input.
    ...
    public class QuantityMeasurementController : IQuantityMeasurementApp
    {
        ...
        private static IQuantityMeasurementRepository SelectRepository(IConfiguration config)
        {
            while (true)
            {
                ...
                if (choice == "1")
                {
                    Console.WriteLine("\n[Program] Cache Repository selected V");
                    Console.WriteLine("[ApplicationConfig] Repository type from menu selection: cache");
                    Console.WriteLine("[App] Using In-Memory Cache Repository.");
                    return QuantityMeasurementCacheRepository.Instance;
                }
                else if (choice == "2")
                {
                    Console.WriteLine("\n[Program] Database Repository selected V");
                    Console.WriteLine("[ApplicationConfig] Repository type from menu selection: database");
        ...
        // ════════════════════════════════════════════════════════════════
        // HISTORY MENU (submenu: options 7-11)
        // ════════════════════════════════════════════════════════════════
