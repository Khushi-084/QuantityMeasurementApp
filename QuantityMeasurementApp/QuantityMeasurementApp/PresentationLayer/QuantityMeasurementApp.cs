...
namespace QuantityMeasurementApp.PresentationLayer
{
    class QuantityMeasurementApp
    {
        ...
        /// <summary>
        /// Reads a volume unit from the console, retrying until valid input is given.
        /// Accepts: l/litre/liter, ml/millilitre/milliliter, gal/gallon
        /// </summary>
        ...
        /// <summary>
        /// Reads a volume value and unit from the console with full validation:
        ///   - Must be a valid number
        ///   - Must be finite (no NaN / infinity)
        ///   - Absolute value must be within 0 to MaxVolumeValue (100,000)
        ///   - Unit must be Litre, Millilitre, or Gallon
        /// Retries on every invalid input instead of returning null.
        /// </summary>
        private static QuantityVolume ReadQuantityVolume(string which)
        {
            while (true)
            {
                // ── Step 1: read and validate the numeric value ──────────
                ...
                // ── Step 2: read and validate the unit ───────────────────
                VolumeUnit unit = ReadVolumeUnit(
                    $"Enter {which} unit (Litre / Millilitre / Gallon): ");

                // ── Step 3: construct — catches any remaining edge cases ──
                try
                {
